...
class PYOD():
    def __init__(self, seed, model_name, tune=False):
        ...
        self.model_dict = {'IForest':IForest, 'OCSVM':OCSVM, 'ABOD':ABOD, 'CBLOF':CBLOF, 'COF':COF, 'AOM':aom,
                           ...
                           'SOGAAL': SO_GAAL, 'MOGAAL': MO_GAAL}
        if AutoEncoder is not None:
            self.model_dict['AutoEncoder'] = AutoEncoder
        if XGBOD is not None:
            self.model_dict['XGBOD'] = XGBOD
    ...
    def fit(self, X_train, y_train, ratio=None):
        ...
        if best_param is not None:
            if self.model_name == 'IForest':
                ...
                self.model = self.model_dict[self.model_name](n_features=X_train.shape[1], epochs=best_param).fit(X_train)
            ...
            # if self.model_name == 'IForest':
            #     self.model = self.model_dict[self.model_name](n_estimators=100).fit(X_train, y_train)
            # elif self.model_name == 'LOF':
            #     self.model = self.model_dict[self.model_name](n_neighbors=20).fit(X_train, y_train)
            # else:
            ...
            if self.model_name == 'DeepSVDD':
                self.model = self.model_dict[self.model_name](n_features=X_train.shape[1]).fit(X_train, y_train)
            else:
                self.model = self.model_dict[self.model_name]().fit(X_train, y_train)
